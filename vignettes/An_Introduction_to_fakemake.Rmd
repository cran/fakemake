...
Vignettes are built using *knitr*, which itself uses `sink()`. As `sink()` is 
...
```
If you wonder why the only prerequisite for `testthat` shown in the above graphs 
is `R/throw.R`: there are no files to be found under `tests/` 
or `inst/` yet. We will come back to this later.
...
quite some CPU time compared to unconditionally reruning the codes in the 
makelist:
...
Now we see - besides other things - that `testthat` has two new prerequisites: 
`tests/testthat.R` and 
